                  ...
                  if (err){
                    console.log(err);
                      ...
                      if(req.user.Role=="User")
                        res.render("home", {loggedIn: 1, page: 1, info1: info1, info2: info2, product1: product1, product2: product2});
...
app.get("/categoryShop/:category",(req,res)=>{
  if (req.isAuthenticated()){
    if(req.user.Role=="User"){

        var category1 = req.params.category;
        Product_farmer.find({Category:category1}, function (err, product1) {
          if (err){
            console.log(err);
          }
          else{
            Product_shopkeeper.find({Category:category1}, function (err, product2) {
              if (err){
                console.log(err);
              }
              else{
                farmCart.find({ID1:ID,Category:category1},(err,list1)=>{
                  if(err){
                    console.log(err)
                  }
                  else{
                    shopCart.find({ID1:ID,Category:category1},(err,list2)=>{
                      if(err){
                        console.log(err)
                      }
                      else{
                        ...
                        // console.log(product1,product2,list1);
                        res.render("shop", {categories:categories,msg:msg,loggedIn: 1, page: 3, product1: product1, product2: product2,p1:p1,p2:p2});
                      }
                    })
  ...
  if (req.isAuthenticated()){
    ...
    if(req.user.Role=="User"){
      ...
      for(var i=0;i<orders.length;i++){
        indOrders = [];
        let orderDetail = await orderDetails.find({ID3:orders[i]._id});
        if(orderDetail[i].ID1!=null){
          let shop = await Product_shopkeeper.find({_id:orderDetail[i].ID1});
          indOrders.push(shop);
        }
        else{
          let farm = await Product_farmer.find({_id:orderDetail[i].ID2});
          indOrders.push(farm);
        }
        userOrders.push(indOrders);
        let delivery = await assignTable.find({ID2:orders[i]._id})
        status.push(delivery[0].status);
      }

      res.render("orderHistory",{userOrders:userOrders,orders:orders,status:status});
    }
    else{
      res.redirect("/");
    }
        ...
        shopCart.find({ID1:ID},(err,list2)=>{
          if(err){
            ...
            for(var i=0;i<list2.length;i++)
              ...
              res.render("checkout", {loggedIn: 1, page: 7,cart1: list1,cart2:list2,cost});
...
app.post("/Review", function(req, res){
  ...
  var r;
  if(types==1){
    Review.find({ID2: i}, function(err, review){
      if(err){
        console.log(err);
      }
      else{
        r=review[0].Rating;
        console.log("R", r+rating);
        r=(Number(rating)+Number(r))/2;
        console.log(r);
        var update = { $set: 
          { 
            Rating: r
        ...
        Review.updateOne(query, update, options, function(error){if(error){console.log(error);}else{console.log("Sucessfully updated");};});
        res.redirect("/Review/"+i);
      }
    });
  }
  else{
    Review.find({ID1: i}, function(err, review){
      if(err){
        console.log(err);
      }
      else{
...
app.get("/Review", function(req, res){
  Review.find({ ID1: id_product}, function (err, review) {
    if (err){
      console.log(err);
    }
    else{
      ...
      id_product=null; 
      console.log(i);
      Product_shopkeeper.find({_id: i}, function(err, product){
        if (err){
          console.log(err);
        }
        else{
          if(review[0]!=null&&product[0]!=null){
            let r=review[0].Rating;
            res.render("Review", {review: r, product: product[0], i: 2, page: 13, loggedIn: 1});
          }
        }
      });
  }});
  Review.find({ ID2: id_product}, function (err, review) {
      if (err){
        console.log(err);
      }
      else{
        ...
        id_product=null; 
        console.log(i);
        Product_farmer.find({_id: i}, function(err, product){
          if (err){
            console.log(err);
          }
          else{
            if(review[0]!=null&&product[0]!=null){
              let r=review[0].Rating;
              res.render("Review", {review: r, product: product[0], i: 1, page: 13, loggedIn: 1});
            }
          }
        });
  }});
...
app.post("/delivered",async (req,res)=>{
  if (req.isAuthenticated()){
    ...
    var obj = Object.entries(req.body);
    var status = await assignTable.findOneAndUpdate({_id: obj[0][0]}, {status:true});
                      ...
                      Product_shopkeeper.find({}, function (err, product2) {
                        if (err){
                          ...
                          if(req.user.Role=="Farmer")
                            ...
                            res.render("Order", {ordDeatils: ordDetails, loggedIn: 3, page: 12, products: products, product1: product1, product2: product2});
                        }
                      });
                    }
                  });
...
app.post("/Add_products_farmer", upload, function(req, res){
  if (req.isAuthenticated()){
    ...
    if(add!=0){
      var query = { _id: add};
      var options = { upsert: true };
      Product_farmer.updateOne(query, update, options, function(error){if(error){console.log(error);}else{console.log("Sucessfully updated");};});
    }
    else{
...
app.post("/Add_products_shopkeeper", upload, function(req, res){
  if (req.isAuthenticated()){
    ...
    if(add!=0){
      ...
      var options = { upsert: true };
      Product_shopkeeper.updateOne(query, update, options, function(error){if(error){console.log(error);}else{console.log("Sucessfully updated");};});
    }
    else{
